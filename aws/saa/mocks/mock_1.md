...
- Identity Federation: アプリユーザーがS3などのAWSのリソースに触る必要があるアプリがあります。そのユーザーにはS3へのアクセス権が必要です。でも大量にいるユーザーごとにIAMユーザー作ってアクセスキー配布なんてことはできません。代わりにAmazonやSNS、Googleのユーザーでサインインしてもらい、その時に外部サービスのユーザー情報とIAMロールを紐づけて、一時的にアクセス権限を付与する機能。

...
- DevOps エンジニアは、Auto Scaling グループの背後にあるすべての EC2 インスタンスがユーザーリクエストに応答しないことに気づきました。これらのインスタンスはターゲットグループの HTTP ヘルスチェックにも失敗しています。ログには大量のメモリ不足のメッセージが記録されており、アプリケーションが実行されていない状態です。アプリケーションのメモリリークに対処し、アプリケーションの耐障害性を向上させるための監視と通知が必要です。
  ...
  - Auto Scaling グループの設定を更新し、ELB ヘルスチェックを有効にします。これにより、EC2 ステータスチェックと ELB ヘルスチェックのいずれかに失敗したインスタンスが置き換えられます。
...
- CodeDeploy で EC2 をデプロイする詳細なプロセスステップ
  ...
  - ネットワークフローの詳細 - インターネットアクセスの確保:パブリックサブネットに配置された EC2 インスタンスは直接インターネットアクセスを持ちます。
    プライベートサブネットに配置された EC2 インスタンスは、NAT ゲートウェイを通じてインターネットアクセスを持ちます。 - セキュリティグループ設定: EC2 インスタンスのセキュリティグループで、アウトバウンドトラフィックとしてポート 443（HTTPS）を許可します。これにより、CodeDeploy エンドポイントおよび S3 バケットへのアクセスが可能になります。 - 問題解決のための設定 - ネットワーク設定:EC2 インスタンスにパブリック IP を割り当てる。セキュリティグループでポート 443（HTTPS）を許可する。プライベートサブネットの場合、NAT ゲートウェイを使用する。 - IAM インスタンスプロファイルの設定: EC2 インスタンスが CodeDeploy と通信するために必要な権限を持つ IAM ロールを設定する。
...
- 監査、分析、トラブルシューティングの目的で、データ分析アプリケーションの DevOps エンジニアは、終了前に Amazon EC2 インスタンスからアプリケーションと Linux システムのログをすべて収集する必要があります。同社は、平均して、Auto Scaling グループで 10,000 のインスタンスを実行しています。同社は、インスタンス ID と日付範囲に基づいてログをすばやく見つける機能が必要です。最も費用対効果の高いソリューションを選択してください。
  ...
  - Amazon EC2 Auto Scaling ライフサイクルアクションは Amazon EventBridge （Amazon CloudWatch Events） をトリガーするように設定でき、AWS Lambda 関数はすべてのログを Amazon S3 にプッシュして、Amazon DynamoDB にカタログを作成します。こ
